...
class Mouse(object):
    """ Mouse monitoring and control functions.
        cmd_signal: a function or keystroke.
        onlick/onmove: a function or python statement string for eval().
        TODO: Currently works for windows mouse only
    """

    def __init__(self, cmd=27, oncmd=None, onclick=None, onmove=None):
    ...
    def _move_watch(self):
        ...
        def _detected(event):
            if event.Message == 'mouse move':
                print(event)
                self._detected(event.Position)
            return True

        hmouse = pyHook.HookManager()
        hmouse.MouseAll = _detected
        hmouse.HookMouse()
        pythoncom.PumpMessages()
